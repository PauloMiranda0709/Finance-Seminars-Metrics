...
startingvalues=[mean(returns);log(var(returns));0.40;0.88;0.0;6]; 
...
lowerbound = [-inf,-inf,-1,-inf,0.0,2]; % mu, lambda can be anything,  should be positve, we want nu>2
upperbound = [inf,inf,1,inf,0.0,40]; % upper bound for nu is 40, which is high enough to resemble a normal distribution
...
[sigmas, u, v]   = Filter_BetaGARCH(mu_hat,lambda_hat,phi_hat,kappa_hat,kappa_tilde_hat, nu_hat,returns);

%% Plot the data along with the output of the GARCH filter
figure
plot(dates,returns,'k')
hold on
plot(dates,sigmas,'r:','linewidth',2)
...
ylim([-15,15]);
set(gca, 'FontName', 'Times', 'fontsize', 20, 'TickDir', 'out');
legend('data','Estimated $\sigma_{t|t-1}$ from Beta GARCH model','Interpreter','latex')
...
hold on
plot(dates,sigmas.^2,'r:','linewidth',2)
...
legend('Squared returns', 'Realised variance','Rescaled version of VIX$^2$','Estimated $\sigma^2_{t|t-1}$ from Beta GARCH model','Interpreter','latex')
